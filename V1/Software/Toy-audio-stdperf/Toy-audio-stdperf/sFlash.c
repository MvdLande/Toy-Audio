...
uint8_t SPI_Transfer8(uint8_t byte)
{
	...
	// Write data before the rising edge of CLK
	// Read data before the falling edge of CLK
	int8_t i; 	
	// write MOSI CLK = LOW
	for (i = 7; i >= 0; i--) //MSB first
	{
		...
		if (((byte >> i) & 0x01) == 0x01)
			...
			sFLASH_MOSI_LOW();
		sFLASH_CLK_HIGH();	//write MOSI bit
		temp = sFLASH_MISO_READ(); // read MISO bit
		spi_data |= (temp << i);
	}
	return spi_data;
}
...
void sFLASH_LowLevel_BitBangInit(void);
...
void sFLASH_LowLevel_BitBangInit(void)
{
	...
	// SPI1 Master initialization, SCK -> PA5
	...
	// SPI1 MASTER initialization, MISO -> PA6
	GPIO_StructInit(&GPIO_InitStructure);
	...
	// SPI1 Master initialization, MOSI -> PA7
	GPIO_StructInit(&GPIO_InitStructure);
	...
	// CS -> PF0
	GPIO_StructInit(&GPIO_InitStructure);
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; // nCS pin
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; 
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
{
	GPIO_InitTypeDef GPIO_InitStructure;
	//RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);// for interrupts etc.  
	...
	// SPI1 MASTER initialization, MISO -> PA6
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource6, GPIO_AF_0); //SPI1
	// SPI1 Master initialization, MOSI -> PA7
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource7, GPIO_AF_0); //SPI1
...
/**
  ...
  * @retval None
  */
void sFLASH_BitBangInit(void)
{

	sFLASH_LowLevel_BitBangInit();
    
	/*!< Deselect the FLASH: Chip Select high */
	sFLASH_CS_HIGH();
	sFLASH_CLK_HIGH(); //default idle state SPI MODE 3
...
void sFLASH_Init(void)
{
	...
	SPI_DataSizeConfig(SPI1, SPI_InitStructure.SPI_DataSize);
	SPI_CalculateCRC(SPI1, DISABLE);
	// http://stackoverflow.com/questions/22769920/stm32f0-spi-loopback-no-data-on-miso
	...
	// So, if you want the flag SPI_I2S_FLAG_RXNE to be set when only 1 byte is received, you should use SPI_RxFIFOThreshold_QF
...
uint8_t sFLASH_Verify(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
{
  /*!< Select the FLASH: Chip Select low */
	...
	{
	  /*!< Read a byte from the FLASH */
		if (*pBuffer == sFLASH_SendByte(sFLASH_DUMMY_BYTE))
		{
		/*!< Point to the next location where the byte read will be read*/
			pBuffer++;			
		}
		else
			return 1;
